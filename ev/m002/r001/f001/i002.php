...
/**/
$FUNCOES->consulta(array ( "tabelas" 	=> " dcd_recursos, usuarios ", "condicoes" 	=> " login = usuarioRecurso and codigoRecurso = $codigoRecurso " ) );
...
$FUNCOES->consulta(array
			...
			"campos" 	=> "r1.codigoRecurso, u1.nome, c1.descricaoCargo, c1.deflatorCargo, 
							...
							(select count(1) from dcd_frentes where codigoRecurso = r1.codigoRecurso) numeroFrentes",
...
if (is_array($filtro))	{
	foreach ($filtro as $key => $value){
		...
		if ($value == 9){ /* "Concluido" */
			if ($filtroQuery<>""){
				$filtroQuery.= " $condicao f1.codigoFase = 11 "; 
			} else{
				$filtroQuery.= " f1.codigoFase = 11 "; 
			}
		}
	}
} else {
...
if (count($filtro)>1) {
	If (!isset($filtroBusca[9])) {
		$filtroQuery = " and ( $filtroQuery ) and f1.codigoFase <> 11 ";
	}else {
...
if ($FUNCOES->GetLinhas()>0)
{
	...
	while ($obj=mysql_fetch_object($FUNCOES->GetResultado()))
	{
		...
		if ($obj->codigoFase == 11) 		{ $comentario.=" <strong style=\"font-size: 10; color: red;\">[Fentre Concluida]</strong>"; 				} 
		$qnt=$qnt+1;
		...
		$listaTabela[]="
				<tr style=\"cursor: pointer;\" onclick=\"document.aplicacao.codigoProjeto.value='$obj->codigoProjeto'; document.aplicacao.codigoFrente.value='$obj->codigoFrente'; executar('m001/r001/f001/loadFrente','aplicacao')\">
					...
					<td>".($obj->nomeFrente)."<br>$comentario</td>
